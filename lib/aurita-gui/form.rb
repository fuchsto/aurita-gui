...
module GUI
  ...
  class Form < Element
    ...
    # Array of Form_Field instances included in this form. 
    ...
    # Argument index may be an integer or a form field name. 
    # 
    ...
    def set_values_if_missing(value_hash={})
      ...
      @values.each_pair { |field_name, value|
        element = @element_map[field_name.to_s]
        if element then
          if element.value.to_s == '' then
            element.value = value 
            @values[field_name] = value_hash[field_name]
          end
        end
      }
    end
